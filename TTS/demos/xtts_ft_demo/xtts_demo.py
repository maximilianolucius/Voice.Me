...
# Global model reference; set by load_model() after training, read by run_tts().
# This module-level mutable state is required by the Gradio callback wiring.
XTTS_MODEL = None
...
def run_tts(lang, tts_text, speaker_audio_file):
    ...
    if not speaker_audio_file:
        return "You need to provide a speaker reference audio!", None, None
    if not os.path.isfile(speaker_audio_file):
...
# redirect stdout to a file for Gradio log display
sys.stdout = Logger()
# Don't redirect stderr to the Logger - keep it separate so tracebacks
# are always visible on the real stderr stream.
