"""Voice.Me TTS model manager.

Provides the :class:`ModelManager` class for listing, downloading, and
managing pre-trained TTS, vocoder, and voice conversion models defined
in a ``.models.json`` registry file.
"""
...
LICENSE_URLS = {
    ...
    "cpml": "https://voiceme.ai/cpml.txt",
}
...
class ModelManager(object):
    ...
    Provides an interface to list, query, download, and cache pre-trained models
    from remote sources. Models are downloaded under a ``tts`` subdirectory in
    the user's data directory.

    Attributes:
        tqdm_progress: Shared tqdm progress bar instance for downloads.
        progress_bar (bool): Whether to display a progress bar during downloads.
        verbose (bool): Whether to print informational messages.
        output_prefix (str): Local directory prefix for downloaded models.
        models_dict (dict): Parsed model registry from the JSON file.

    Args:
        models_file (str): Path to .models.json file. Defaults to None.
        output_prefix (str): Prefix path for downloading models. Defaults to None.
        progress_bar (bool): Print a progress bar when downloading a file. Defaults to False.
    ...
    def download_fairseq_model(self, model_name, output_path):
        URI_PREFIX = "https://models.voiceme.ai/fairseq/"
    ...
    def set_model_url(model_item: Dict):
        ...
        if "github_rls_url" in model_item:
            ...
            model_item["model_url"] = "https://models.voiceme.ai/fairseq/"
        elif "xtts" in model_item["model_name"]:
            model_item["model_url"] = "https://models.voiceme.ai/xtts/"
        return model_item
    ...
    def _set_model_item(self, model_name):
        ...
        if "fairseq" in model_name:
            ...
            model_item = {
                "default_vocoder": None,
                "license": "CPML",
                "contact": "info@voiceme.ai",
                "tos_required": True,
                "hf_url": [
                    f"https://models.voiceme.ai/hf/XTTS-v2/{model_version}/model.pth",
                    f"https://models.voiceme.ai/hf/XTTS-v2/{model_version}/config.json",
                    f"https://models.voiceme.ai/hf/XTTS-v2/{model_version}/vocab.json",
                    f"https://models.voiceme.ai/hf/XTTS-v2/{model_version}/hash.md5",
                    f"https://models.voiceme.ai/hf/XTTS-v2/{model_version}/speakers_xtts.pth",
                ],
            }
        else:
    ...
    def create_dir_and_download_model(self, model_name, model_item, output_path):
        ...
        try:
            ...
            logger.error(" > Failed to download the model file to %s", output_path)
            rmtree(output_path)
            raise e
    ...
    def _download_zip_file(file_url, output_folder, progress_bar):
        ...
        try:
            ...
            logger.error(" > Error: Bad zip file - %s", file_url)
    ...
    def _download_tar_file(file_url, output_folder, progress_bar):
        ...
        try:
            ...
            logger.error(" > Error: Bad tar file - %s", file_url)
    ...
    def _download_model_files(file_urls, output_folder, progress_bar):
        ...
        for file_url in file_urls:
            ...
            temp_zip_name = os.path.join(output_folder, base_filename)
            total_size_in_bytes = int(r.headers.get("content-length", 0))
            block_size = 1024  # 1 Kibibyte
            with open(temp_zip_name, "wb") as file:
                if progress_bar:
