"""Voice.Me TTS synthesizer module.

Provides the :class:`Synthesizer` class that orchestrates TTS and vocoder models
to generate speech from text, including support for multi-speaker, multi-lingual,
voice conversion, and style transfer scenarios.
"""
...
class Synthesizer(nn.Module):
    """General Voice.Me TTS synthesizer for inference.

    Orchestrates TTS, vocoder, speaker encoder, and voice conversion models to
    generate speech from text. Text is split into sentences for individual
    synthesis and then concatenated.

    Attributes:
        tts_model: Loaded TTS model instance.
        vocoder_model: Loaded vocoder model instance, or None for Griffin-Lim.
        vc_model: Loaded voice conversion model instance, or None.
        output_sample_rate (int): Sample rate of the generated audio.
        use_cuda (bool): Whether CUDA is enabled.
    """
    ...
    def __init__(
        ...
        """Initialize the Synthesizer with model paths and configuration.

        Takes a TTS and an optional vocoder model and synthesizes speech from the
        provided text. The text is divided into sentences using ``pysbd`` and each
        sentence is synthesized separately.
        ...
        Args:
            ...
            tts_speakers_file (str, optional): Path to the speakers JSON file. Defaults to ``""``.
            tts_languages_file (str, optional): Path to the languages JSON file. Defaults to ``""``.
            vocoder_checkpoint (str, optional): Path to the vocoder model file. Defaults to ``""``.
            vocoder_config (str, optional): Path to the vocoder config file. Defaults to ``""``.
            ...
            model_dir (str, optional): Directory containing model files for self-loading models. Defaults to ``""``.
            voice_dir (str, optional): Directory with external voice files for cloning. Defaults to None.
    ...
    def voice_conversion(self, source_wav: str, target_wav: str) -> List[int]:
        """Convert the voice in source_wav to match the target speaker.

        Args:
            source_wav (str): Path to the source audio file.
            target_wav (str): Path to the target speaker audio file.

        Returns:
            List[int]: Converted waveform.
        """
    ...
    def tts(
        ...
        **kwargs,
        ...
        """Run all the models and generate speech.
        ...
        if not text and not reference_wav:
            raise ValueError(
                "You need to define either `text` (for sythesis) or a `reference_wav` (for voice conversion) to use the Voice.Me API."
            )

        if text:
            sens = [text]
            if split_sentences:
                logger.info(" > Text split to sentences.")
                sens = self.split_into_sentences(text)
            logger.debug(" > Sentences: %s", sens)
        ...
        logger.info(" > Processing time: %s", process_time)
        logger.info(" > Real-time factor: %s", process_time / audio_time)
        return wavs
