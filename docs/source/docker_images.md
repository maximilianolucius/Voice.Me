...
#### CPU version
```bash
docker pull ghcr.io/voiceme-ai/tts-cpu
```
#### GPU version
```bash
docker pull ghcr.io/voiceme-ai/tts
```
...
### CPU version
```bash
docker run --rm -v ~/tts-output:/root/tts-output ghcr.io/voiceme-ai/tts-cpu --text "Hello." --out_path /root/tts-output/hello.wav
```
### GPU version
...
```bash
docker run --rm --gpus all -v ~/tts-output:/root/tts-output ghcr.io/voiceme-ai/tts --text "Hello." --out_path /root/tts-output/hello.wav --use_cuda true
```
...
### CPU version
```bash
docker run --rm -it -p 5002:5002 --entrypoint /bin/bash ghcr.io/voiceme-ai/tts-cpu
...
### GPU version
```bash
docker run --rm -it -p 5002:5002 --gpus all --entrypoint /bin/bash ghcr.io/voiceme-ai/tts
python3 TTS/server/server.py --list_models #To get the list of available models
