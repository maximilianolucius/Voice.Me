# Fine-tuning a Voice.Me TTS model

## Fine-tuning

Fine-tuning takes a pre-trained model and retrains it to improve the model performance on a different task or dataset.
In Voice.Me TTS we provide different pre-trained models in different languages and different pros and cons. You can take one of
...
## Steps to fine-tune a Voice.Me TTS model

1. Setup your dataset.

    You need to format your target dataset in a certain way so that Voice.Me TTS data loader will be able to load it for the
